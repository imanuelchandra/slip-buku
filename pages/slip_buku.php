...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	$bookslip_address_text = ''; // tuliskan alamat perpustakaan dalam tanda petik 
	$bookslip_header_text = ''; // biarkan kosong jika nama perpustakaan sesuai nama perpustakaan pada SLiMS
	...
	foreach ($chunked_book_card_arrays as $bookslip_rows) {
		...
		foreach ($bookslip_rows as $bookslip) {
			$html_str .= '<td valign="top">'."\n";
				...
				$html_str .= '<td valign="top" style="margin: 0;">'.$bookslip[2].'</td>'."\n";
				$html_str .= '</tr>'."\n";
				$html_str .= '<tr>'."\n";
				...
				$html_str .= '<td valign="top" style="margin: 0; text-align: center; width: 3.5cm;">Tgl. Harus Kembali</td>'."\n";
...
<div class="menuBoxInner printIcon">
    ...
    <div class="btn-group">
      <a target="blindSubmit" href="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery(['action' => 'clear']) ?>" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
      <a target="blindSubmit" href="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery(['action' => 'print']) ?>" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Cetak Slip Buku Dari Data Terpilih'); ?></a>
	</div>
    <form name="search" action="<?= $_SERVER['PHP_SELF'] . '?' . httpQuery() ?>" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
